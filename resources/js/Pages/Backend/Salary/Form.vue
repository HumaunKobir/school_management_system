...
<script setup>
    ...
    const props = defineProps(['salary', 'id']);

    const form = useForm({
        name: props.salary?.name ?? '',
        _method: props.salary?.id ? 'put' : 'post',
    });
    ...
    const submit = () => {
        const routeName = props.id ? route('backend.salary.update', props.id) : route('backend.salary.store');
        form.transform(data => ({
            ...data,
            remember: '',
            isDirty: false,
