...
<script setup>
    ...
    const props = defineProps(['assignment', 'id']);

    const form = useForm({
        name: props.assignment?.name ?? '',
        _method: props.assignment?.id ? 'put' : 'post',
    });
    ...
    const submit = () => {
        const routeName = props.id ? route('backend.assignment.update', props.id) : route('backend.assignment.store');
        form.transform(data => ({
            ...data,
            remember: '',
            isDirty: false,
