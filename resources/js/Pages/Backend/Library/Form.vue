...
<script setup>
    ...
    const props = defineProps(['library', 'id']);

    const form = useForm({
        name: props.library?.name ?? '',
        _method: props.library?.id ? 'put' : 'post',
    });
    ...
    const submit = () => {
        const routeName = props.id ? route('backend.library.update', props.id) : route('backend.library.store');
        form.transform(data => ({
            ...data,
            remember: '',
            isDirty: false,
