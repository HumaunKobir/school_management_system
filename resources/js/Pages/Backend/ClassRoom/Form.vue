...
<script setup>
    ...
    const props = defineProps(['classroom', 'id']);

    const form = useForm({
        room_number: props.classroom?.room_number ?? '',
        capacity: props.classroom?.capacity ?? '',
        _method: props.classroom?.id ? 'put' : 'post',
    });
    ...
    const submit = () => {
        const routeName = props.id ? route('backend.classroom.update', props.id) : route('backend.classroom.store');
        form.transform(data => ({
            ...data,
            remember: '',
            isDirty: false,
            ...
            <div
                ...
                <form @submit.prevent="submit" class="p-4">
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div class="col-span-1 md:col-span-1">
                            <InputLabel for="capacity" value="Room Capacity" />
                            <input id="capacity"
                                class="block w-full p-2 text-sm rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600"
                                v-model="form.capacity" type="number" placeholder="Room Capacity" />
