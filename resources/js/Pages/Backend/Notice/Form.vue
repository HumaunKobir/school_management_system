...
<script setup>
    ...
    const props = defineProps(['notice', 'id']);

    const form = useForm({
        name: props.notice?.name ?? '',
        _method: props.notice?.id ? 'put' : 'post',
    });
    ...
    const submit = () => {
        const routeName = props.id ? route('backend.notice.update', props.id) : route('backend.notice.store');
        form.transform(data => ({
            ...data,
            remember: '',
            isDirty: false,
