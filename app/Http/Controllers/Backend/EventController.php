...
class EventController extends Controller
{
    ...
    public function store(EventRequest $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('image'))
                $data['image'] = $this->imageUpload($request->file('image'), 'events');

            if ($request->hasFile('file'))
                $data['file'] = $this->fileUpload($request->file('file'), 'events');
    ...
    public function update(EventRequest $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                $data['image'] = $this->imageUpload($request->file('image'), 'events');
                $path = strstr($event->image, 'storage/');
                if (file_exists($path)) {
                    unlink($path);
                }
            } else {

                $data['image'] = strstr($event->image ?? '', 'events');
            }

            if ($request->hasFile('file')) {
                $data['file'] = $this->fileUpload($request->file('file'), 'events/');
                $path = strstr($event->file, 'storage/');
                if (file_exists($path)) {
                    unlink($path);
                }
            } else {

                $data['file'] = strstr($event->file ?? '', 'events/');
            }
            ...
            DB::rollBack();
            $this->storeSystemError('Backend', 'EventController', 'update', substr($err->getMessage(), 0, 1000));
            DB::commit();
            $message = "Server Errors Occur. Please Try Again.";
            return redirect()
                ->back()
    ...
    public function changeStatus(Request $request, $id, $status)
    {
        ...
        try {
            ...
            $dataInfo = $this->eventService->changeStatus($id, $status);
            ...
            DB::rollBack();
            $this->storeSystemError('Backend', 'EventController', 'changeStatus', substr($err->getMessage(), 0, 1000));
            DB::commit();
            $message = "Server Errors Occur. Please Try Again.";
            return redirect()
                ->back()
