...
<script setup>
    ...
    const props = defineProps(['group', 'id','classes']);

    const form = useForm({
        class_id: props.group?.class_id ?? '',
    ...
    const submit = () => {
        const routeName = props.id ? route('backend.group.update', props.id) : route('backend.group.store');
        form.transform(data => ({
            ...data,
            remember: '',
            isDirty: false,
                ...
                <form @submit.prevent="submit" class="p-4">
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div class="col-span-1 md:col-span-1">
                            ...
                            <input id="name"
                                class="block w-full p-2 text-sm rounded-md shadow-sm border-slate-300 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-200 focus:border-indigo-300 dark:focus:border-slate-600"
                                v-model="form.name" type="text" placeholder="Group Name" />
