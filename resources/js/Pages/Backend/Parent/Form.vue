...
<script setup>
    ...
    const props = defineProps(['parent', 'id']);

    const form = useForm({
        name: props.parent?.name ?? '',
        _method: props.parent?.id ? 'put' : 'post',
    });
    ...
    const submit = () => {
        const routeName = props.id ? route('backend.parent.update', props.id) : route('backend.parent.store');
        form.transform(data => ({
            ...data,
            remember: '',
            isDirty: false,
