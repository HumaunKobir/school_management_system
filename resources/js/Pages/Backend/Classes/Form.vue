...
<script setup>
    ...
    const props = defineProps(['classes', 'id']);

    const form = useForm({
        name: props.classes?.name ?? '',
        _method: props.classes?.id ? 'put' : 'post',
    });
    ...
    const submit = () => {
        const routeName = props.id ? route('backend.classes.update', props.id) : route('backend.classes.store');
        form.transform(data => ({
            ...data,
            remember: '',
            isDirty: false,
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div class="col-span-1 md:col-span-1">
                            ...
                            <input id="name"
                                ...
                                v-model="form.name" type="text" placeholder="Class Name" />
