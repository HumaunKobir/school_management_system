...
<script setup>
    ...
    const props = defineProps(['scholarship', 'id']);

    const form = useForm({
        name: props.scholarship?.name ?? '',
        _method: props.scholarship?.id ? 'put' : 'post',
    });
    ...
    const submit = () => {
        const routeName = props.id ? route('backend.scholarship.update', props.id) : route('backend.scholarship.store');
        form.transform(data => ({
            ...data,
            remember: '',
            isDirty: false,
